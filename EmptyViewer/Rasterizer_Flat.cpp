...
void rasterize_triangle(const vec4& v0, const vec4& v1, const vec4& v2, const ivec3& tri) {
    ...
    vec4 mv0 = view * model * vec4(vertices[tri.x], 1.0f);
    vec4 mv1 = view * model * vec4(vertices[tri.y], 1.0f);
    vec4 mv2 = view * model * vec4(vertices[tri.z], 1.0f);

    vec3 wp0 = vec3(mv0);
    vec3 wp1 = vec3(mv1);
    vec3 wp2 = vec3(mv2);
    vec3 centroid = (wp0 + wp1 + wp2) / 3.0f;
    vec3 normal = normalize(cross(wp1 - wp0, wp2 - wp0));
    vec3 color = compute_lighting(centroid, normal);
